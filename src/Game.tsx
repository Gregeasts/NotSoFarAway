...
type GameMessage =
  ...
  | { type: 'chat'; text: string }; // optional future messages
...
export default function Game({ roomId }: { roomId: string }) {
  ...
  useEffect(()=>{
    // Setup WebRTC
    const conn = new WebRTCConnection(roomId);
    ...
    function create(this: Phaser.Scene) {
      const makeDragon = (color:number) => {
        ...
        c.add([body, head, eye]); return c;
      };
      mySprite = makeDragon(0x8fd3ff); otherSprite = makeDragon(0xffbabf);
